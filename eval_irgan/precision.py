...
def precision_at_k(sess, x_placeholder,gen_scores, query_pos_test, query_pos_train, query_url_feature, k=5):
    p = 0.0
    cnt = 0
    for query in query_pos_test.keys():
        ...
        pred_list_score = sess.run(gen_scores, feed_dict={x_placeholder: pred_list_feature})
...
def precision_at_k_user(sess, gen_scores, query_pos_test, query_pos_train, query_url_feature, k=5):
    p_list = []
    ...
    for query in query_test_list:
        ...
        pred_list_feature = [query_url_feature[query][url] for url in pred_list]
        pred_list_feature = np.asarray(pred_list_feature)
        pred_list_score = sess.run(gen_scores, feed_dict={X: pred_list_feature})
        pred_url_score = zip(pred_list, pred_list_score)
        pred_url_score = sorted(pred_url_score, key=lambda x: x[1], reverse=True)
